...
from model.representation_net import RepresentationNet
from model.prediction_net import PredictionNet
...
class Model:
    ...
    def __init__(self, comp_hiddens, comp_activation,
        ...
        """
        self.comp_net = RepresentationNet(comp_hiddens, comp_activation)
        self.est_net = PredictionNet(est_hiddens, est_activation)
    ...
    def fit(self, x):
        ...
        with tf.Graph().as_default() as graph:
            ...
            # Build graph
            z, z_hat, x_dash  = self.comp_net.inference(input)
            self.z_hat = z_hat
            gamma = self.est_net.inference(z_hat, drop)
            self.gmm.fit(z_hat, gamma)
            energy = self.gmm.energy(z_hat)

            self.x_dash = x_dash

            # for KDD 1 1 0.1 0.0001
            loss = (self.comp_net.reconstruction_error(input, x_dash) +
                1*self.comp_net.rsr_error(z, z_hat) + 1*self.comp_net.proj_error()+
                self.lambda1 * tf.reduce_mean(energy) + self.lambda2 * self.gmm.cov_diag_loss())
            ...
            self.energy = self.gmm.energy(z_hat)
