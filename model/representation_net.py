...
class RepresentationNet:
    ...
    def reverse(self, z):
        with tf.variable_scope("Decoder"):
            z = tf.layers.dense(z,15,activation=self.activation)
            n_layer = 0
    ...
    def inference(self, x):
        ...
        with tf.variable_scope("CompNet"):
            # AutoEncoder
            z_c = self.compress(x)
            z_hat = self.rsr(z_c, self.rsr_sizes)
            x_dash = self.reverse(z_hat)
        return z_c,z_hat,x_dash
    ...
    def rsr(self, z, dim):
        A = tf.Variable(tf.random_normal((int(z.get_shape()[-1]), dim)),
                        name="layer_rsr")
        self.A = A
        z_hat = tf.matmul(z, A)
        return z_hat

    def rsr_error(self, z, z_hat):
        z_hat = tf.matmul(z_hat, tf.transpose(self.A))
        return tf.reduce_mean(tf.norm(z-z_hat, ord=2, axis=1))

    def proj_error(self):
        return tf.reduce_mean(tf.square(tf.matmul(tf.transpose(self.A), self.A) - tf.eye(self.rsr_sizes)))